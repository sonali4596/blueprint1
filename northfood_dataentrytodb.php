...
if(isset($_POST['submit'])){
   ...
   // $date_existance=isset($_POST['date_existancedate_existance'])?mysqli_real_escape_string($db,$_POST['date_existance']):null;
    ...
    if($file_error===0){
        ...
        mysqli_query($db,"INSERT INTO `northfood`(`id`, `image`, `description`, `name`, `state_id`, `country`) VALUES (NULL,'$file_newtarget','$description','$name','$state_id','$country')");
        echo" you have entered successfully" ;
        header('location: http://'.$_SERVER['HTTP_HOST']."/blueprint/northfood_form.php");
